...
def _generate_with_gemma(user_input: str, history: List[Dict] = None) -> str:
    """
    Gemma constraints:
    - No 'system' role — system prompt is embedded in the first user message.
    - Strict user→assistant alternation required.
    - apply_chat_template may return a BatchEncoding (newer transformers) or a
      raw tensor (older) — handle both.
    """
    import torch

    # ── Sanitize history: enforce strict alternation, no system role ──────────
    cleaned: List[Dict] = []
    last_role: Optional[str] = None
    for h in (history or [])[-8:]:   # limit to last 8 turns before processing
        raw_role = h.get("role", "")
        ...
        if not content:
            continue
        if role == last_role:
            cleaned[-1] = {"role": role, "content": content}  # merge same-role
        else:
